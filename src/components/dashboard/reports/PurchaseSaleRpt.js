...
const PurchaseSaleRpt = () => {
    ...
    const [report, setReport] = useState({
        purchases: {
            totalPurchase: 0,
            purchaseIncludingTax: 0,
            totalPurchaseReturnIncludingTax: 0,
            purchaseDue: 0,
        },
        sales: {
            totalSale: 0,
            saleIncludingTax: 0,
            totalSellReturnIncludingTax: 0,
            saleDue: 0,
        },
        overall: {
            salePurchase: 0,
    ...
    let summary3 = [];
    for (let section in report) {
        for (let key in report[section]) {
            summary3.push({
                heading: key,
                amount: report[section][key],
            });
        }
    }
    let summary4 = [];
            ...
            <div className='flex flex-col  bg-gray-100 px-3 pb-3 min-h-screen'>
                ...
                <div ref={printRef}>
                    ...
                    <div className='grid grid-cols-2 gap-4 bg-gray-100 mt-3'>
                        <div className='flex flex-col bg-white p-4 w-full'>
                            ...
                            <table className='table-auto mt-3 w-full'>
                                {summary3.map((val, index) => {
                                    return (
                                        <tr className={`${index % 2 === 0 ? "bg-gray-100" : ""} my-2`} key={index}>
                    ...
                    <div className='flex flex-col p-5 mt-5 bg-white'>
                        ...
                        <h1 className='text-2xl text-gray-500 text-start'>
                            Sale - Purchase:
                            <span className='mx-2 text-green-400'>Rs. {report.overall.salePurchase.toFixed(2)}</span>
                        </h1>
                        <h1 className='text-2xl text-gray-500 text-start'>
                            Due Amount:
                            <span className='mx-2 text-green-400'>Rs. {report.overall.dueAmount.toFixed(2)}</span>
                        </h1>
                    </div>
