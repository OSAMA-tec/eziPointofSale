...
import AddorEditWarranties from '../Product/warranties/AddorEditWarranties';
...
const BrandsTbl = () => {
    ...
    const [warraantyData, setWarrantyData] = useState([]);

    const [crpage, setCrpage] = useState(1)
    const rcrdprpg = 5
    const lasIndex = crpage * rcrdprpg
    const frstIndex = lasIndex - rcrdprpg
    const record = warraantyData.slice(frstIndex, lasIndex)
    const npage = Math.ceil(warraantyData.length / rcrdprpg)
    ...
    const csvData = [
        ...
        ...warraantyData.map(({ Username, Name, Role, Email }) => [
            Username,
            Name,
            Role,
            Email
    ...
    const displayData = () => {
        if (isAdd === true) {
            return <AddorEditWarranties id={0} />
        } else if (isEdit === true && iseidtId !== 0) {
            return <AddorEditWarranties id={iseidtId} />
        }
    }
    ...
    return (
        <div>
            <div className='flex justify-between mt-2 text-sm mx-5'>
                ...
                <h1 className='text-xl font-semibold text-start'>All Warranties</h1>
                </div>
            ...
            <div className='flex mt-5 flex-col md:flex-row  items-center justify-center md:justify-between mx-5'>
                ...
                <div className='flex items-center justify-center my-2 md:my-0'>
                    ...
                    <button onClick={() => { handleExportExcl(warraantyData) }} className='flex border-[1px] px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                    ...
                    <button onClick={() => { setColvis(!colvis) }} className='flex border-[1px] relative px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                        ...
                        {colvis && <div className='absolute top-7 shadow-md shadow-gray-400 bg-white w-[150px]'>
                            <ul className='flex flex-col items-center justify-center'>
                                ...
                                <li className={` w-full py-1 ${col3 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol3(!col3) }}>Duration</li>
                ...
                <table id='usertbl' className="table-auto w-full mb-10  whitespace-no-wrap ">
                    <thead>
                        <tr>
                            ...
                            {col3 && <th className=" py-2 title-font  tracking-wider font-medium text-gray-900 text-sm bg-gray-200">Duration</th>}
                    ...
                    <tbody >
                        {record.map((value, index) => {
                            return <tr key={index} className={`${(index + 1) % 2 === 0 ? "bg-gray-100" : ""}`}>
                                ...
                                {col3 && <td className="px-1 py-1"> {value.duration +' '+ value.durationType}</td>}
