...
import AddorEditVariations from '../Product/variations/AddorEditVariations';
...
const VariationsTbl = () => {
    ...
    const record = variationData.slice(frstIndex, lasIndex)
    const npage = Math.ceil(variationData.length / rcrdprpg)
    ...
    const csvData = [
        ...
        ...variationData.map(({ Username, Name, Role, Email }) => [
            Username,
            Name,
            Role,
            Email
    ...
    const displayData = () => {
        if (isAdd === true) {
            return <AddorEditVariations id={0} />
        } else if (isEdit === true && iseidtId !== 0) {
            return <AddorEditVariations id={iseidtId} />
        }
    }
        ...
        <div>
            ...
            <div className='flex mt-5 flex-col md:flex-row  items-center justify-center md:justify-between mx-5'>
                ...
                <div className='flex items-center justify-center my-2 md:my-0'>
                    ...
                    <button onClick={() => { handleExportExcl(variationData) }} className='flex border-[1px] px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                    ...
                    <button onClick={() => { setColvis(!colvis) }} className='flex border-[1px] relative px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                        ...
                        {colvis && <div className='absolute top-7 shadow-md shadow-gray-400 bg-white w-[150px]'>
                            <ul className='flex flex-col items-center justify-center'>
                                <li className={` w-full py-1 ${col1 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol1(!col1) }}>Variations</li>
                                <li className={` w-full py-1 ${col2 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol2(!col2) }}>Values</li>
            ...
            <div className='flex flex-col justify-center items-center mt-5 mx-5' ref={printRef} >
                <table id='usertbl' className="table-auto w-full mb-10  whitespace-no-wrap ">
                    <thead>
                        <tr>
                            {col1 && <th className=" py-2 title-font  tracking-wider font-medium text-gray-900 text-sm bg-gray-200">Variations</th>}
                            {col2 && <th className=" py-2 title-font  tracking-wider font-medium text-gray-900 text-sm bg-gray-200">Values</th>}
                    ...
                    <tbody >
                        {record.map((value, index) => {
                            return <tr key={index} className={`${(index + 1) % 2 === 0 ? "bg-gray-100" : ""}`}>
                                {col1 && <td className="px-1 py-1 text-sm">{value.variationName}</td>}
                                {col2 && <td className="px-1 py-1"> {value.variationValues}</td>}
